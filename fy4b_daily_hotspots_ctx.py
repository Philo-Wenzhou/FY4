# -*- coding: utf-8 -*-
"""
FY-4B | 指定“某一天”的逐小时火点聚合（情境阈值 + 可选Dozier反演）
- 自动配对 GEO/FDI
- LUT 亮温 (CALChannel07/13)
- 文献法：潜在火像元(PFP) + 局地背景(21x21/31x31) + 情境检验（昼/夜分开）
- 灵敏度旋钮 (--sens)：<1 更敏；>1 更保守；默认 0.90（更敏）
...
# MOD14/VIIRS 风格阈值（可被 sens 缩放）
# 昼夜各自的“绝对阈值 + 背景标准差多倍数”混合逻辑
ABS_T4_DAY   = 315.0     # K（昼）
ABS_T4_NGT   = 310.0     # K（夜）
ABS_BTD_DAY  = 10.0      # K
ABS_BTD_NGT  = 8.0       # K

K_T4_DAY   = 3.0         # 背景 σ 倍数
K_T4_NGT   = 2.5
K_BTD_DAY  = 3.0
K_BTD_NGT  = 2.5

OFFS_T4_DAY  = 3.0       # K：最小增量下限（避免σ太小导致过敏）
OFFS_T4_NGT  = 2.0
OFFS_BTD_DAY = 3.5
OFFS_BTD_NGT = 2.5
...
def _local_bg_stats(arr_list, i, j, win):
    """从多张栈(arr_list)里抽取窗口，并排除NaN与极端高值"""
    Hh = win // 2
    ...
    V = np.vstack(vals).T  # [nwin, n_layers]
    return V  # 含NaN，后续用np.nan*处理

def contextual_candidates(T4, T13, sza_deg, satz_deg, sun_glint=None, vis=None, sens=0.90):
    """
    文献法（MOD14/VIIRS思路）：
    1) PFP 潜在火像元（绝对阈值 OR 鲁棒Z）
    ...
    # 1) 潜在火像元（更敏感：降低绝对阈值与z阈值 by sens）
    pfp = np.zeros_like(T4, dtype=bool)

    # 绝对阈值（随 sens 缩放：sens<1 更容易过）
    ...
    pfp |= (day   & earth & ((T4 >= abs_t4_day) | (BTD >= abs_dtd_day) | ((z_t4 >= z_t4_thr) & (z_btd >= z_btd_thr)) ) & (~cloud_day))
    pfp |= (night & earth & ((T4 >= abs_t4_ngt) | (BTD >= abs_dtd_ngt) | ((z_t4 >= z_t4_thr) & (z_btd >= z_btd_thr)) ))

    # 2) 对每个PFP做局地背景 + 3) 情境检验
    Hn, Wn = T4.shape
    ...
    # 若存在强glint（<36°），对“白天的水体或强反射”加严 BTD/T4 增量
    ...
    for y, x in zip(yy, xx):
        ...
        for win in CTX_WIN_LIST:
            V = _local_bg_stats(stack_for_bg, y, x, win)  # shape: [nwin, 3]
            # 排除自身像元附近的极端值：用分位裁掉上1%
...
def planck_radiance_um(T, um):
    ...
    lam = um * 1e-6
    L = (C1 / (lam**5)) / (np.exp(C2 / (lam * (T + 1e-9))) - 1.0)  # W·m^-2·sr^-1·m^-1
...
# ---------------- 单景检测（文献法） ----------------
def detect_once(geo_file, fdi_file, sens=0.90, want_dozier=False, print_stats=False):
    ...
    cand = contextual_candidates(T4, T13, sza_deg, satz_deg, sun_glint=sglit_deg, vis=vis, sens=sens)
    ...
    # 可选：Dozier 需要背景均值，后面小时聚合时再用局地背景估计更稳。
    return cand, T4, T13
...
def run_daily(data_dir, date_str, out_dir,
              ...
              sens=0.90, want_dozier=False, max_resid_px=1.5):
    ...
    if not any_hot:
        print("这一天没有检测到有效热点（文献情境阈值下）。")

# ---------------- CLI ----------------
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="FY-4B | 指定日期逐小时火点聚合（文献法·情境阈值 + 可选Dozier反演）")
    ...
    ap.add_argument("--sens",  type=float, default=0.90, help="灵敏度旋钮：<1 更敏，>1 更保守，默认0.90")
