# -*- coding: utf-8 -*-
# fy4b_hotspot_hourly.py - FY-4B卫星1小时融合火点检测
...
def process_window(data_dir, out_dir, t_start, t_end, k_required=2):
    ...
    if not pairs:
        print("时间窗内没有匹配到 FDI/GEO 文件。"); return

    print(f"窗口 {t_start} ~ {t_end} | 匹配到 {len(pairs)} 对文件")
    nrow, ncol = LAT.shape
...
if __name__ == "__main__":
    ...
    ap.add_argument("--dir",  required=True, help="含 HDF 文件的目录")
    ...
    try:
        t_start = parse_cli_time(args.start)
        t_end = parse_cli_time(args.end)
    except ValueError as e:
        sys.exit(f"时间解析错误: {e}")
    
    if not (t_end > t_start): 
        sys.exit("时间窗无效：end 必须晚于 start")
